...
exports.map = function () {
    var ref = {a: 1, b: 2};
    var items = Hash(ref).map(function (v) {
        return v + 1
    }).items;
    var hash = Hash.map(ref, function (v) {
        return v + 1
    });
...
exports.filter_items = function () {
    ...
    var hash = Hash.filter(ref, function (v, k) {
        return v > 5 || k > 5
    });
...
exports.detect = function () {
    ...
    var hh = Hash(h);
    var gt6hh = hh.detect(function (x) {
        return x > 6
    });
    assert.ok(gt6hh == 7 || gt6hh == 8);
    var gt6h = Hash.detect(h, function (x) {
        return x > 6
    });
    assert.ok(gt6h == 7 || gt6h == 8);
    assert.equal(hh.detect(function (x) {
        return x > 100
    }), undefined);
};
...
exports.some = function () {
    var h = {a: 5, b: 6, c: 7, d: 8};
    var hh = Hash(h);
    assert.ok(Hash.some(h, function (x) {
        return x > 7
    }));
    assert.ok(Hash.some(h, function (x) {
        return x < 6
